module.exports = function(server, ee, test, spotify) {
	spotify = new spotify();
	...
	if(!test){
		...
		io.sockets.on('connection', function(socket) {
			...
			socket.on('api.getSpotifyPlaylists', _io.api.spot.getAllPlaylists);
			socket.on('api.getSpotifyTracks', _io.api.spot.getAllTracks);
			socket.on('api.playTrack', _io.api.spot.playTrack);
		...
		api: {
			...
			spot: {
				playTrack: function(uri, cb){
					console.log(uri);
					spotify.playTrack(uri, cb);
				},
				getAllPlaylists: function(cb){
					spotify.getAllPlaylists(function(d){
						cb({
							playlists: d.slice(0, 15)
						});
					});
				},
				getAllTracks: function(uri, cb){
					spotify.getAllTracks(uri, function(data){
						cb(data);
					})
				}
			},
			weather: function(cb){
				request.get('http://api.openweathermap.org/data/2.5/weather?q=Santa%20Cruz,%20ca', function(r,e,b){
					if(typeof b !== 'undefined'){
						b = JSON.parse(b);
						var ret = {};
						ret.temp = Math.round((1.8 * (parseInt(b.main.temp) - 273) + 32) * 10 ) / 10;
						ret.place = b.name;
						ret.weather = [];
